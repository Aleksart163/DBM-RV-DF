...
local specWarnAncientRadiance				= mod:NewSpecialWarningSpell(413023, nil, nil, DBM_COMMON_L.AOEDAMAGE, 1, 2) --Древнее сияние (АоЕ)
...
local specWarnVolatileMortar				= mod:NewSpecialWarningDodge(407205, nil, nil, DBM_COMMON_L.BOMBING, 2, 2) --Нестабильное орудие (Обстрел)
local specWarnVolatileMortar2				= mod:NewSpecialWarningRun(407205, nil, nil, DBM_COMMON_L.BOMBING, 2, 2) --Нестабильное орудие (Обстрел)
...
local timerTitanticBulwarkCD				= mod:NewCDNPTimer(24.5, 413024, nil, nil, nil, 5, nil, DBM_COMMON_L.TANK_ICON) --Бастион титанов
local timerAncientRadianceCD				= mod:NewCDNPTimer(9.7, 413023, DBM_COMMON_L.AOEDAMAGE, nil, nil, 2) --Древнее сияние (АоЕ) 9.7-15
...
local timerVolatileMortarCD					= mod:NewCDNPTimer(18, 407205, DBM_COMMON_L.BOMBING, nil, nil, 3, nil, DBM_COMMON_L.DEADLY_ICON) --Нестабильное орудие (Обстрел)
...
local yellVolatileMortarFades				= mod:NewShortFadesYell(407205, DBM_COMMON_L.BOMBING, nil, nil, "YELL") --Нестабильное орудие (Обстрел)
...
local function additionalIds(self, args)
	...
	elseif spellId == 407205 then --Не работает
...
do
	local warnedProshlyap = false
	function mod:UNIT_AURA(uId)
		local hasProshlyap = DBM:UnitDebuff("player", 407205)
		if hasProshlyap and not warnedProshlyap then
			warnedProshlyap = true
			specWarnVolatileMortar2:Show()
			specWarnVolatileMortar2:Play("justrun")
			yellVolatileMortar:Yell()
			yellVolatileMortarFades:Countdown(spellId)
		elseif not hasProshlyap and warnedProshlyap then
			warnedProshlyap = false
...
--	elseif cid == 205435 then--epoch-ripper
	...
	elseif cid == 206214 then --Лазутчик из рода Бесконечности
		timerTimelessCurseCD:Stop(args.destGUID)
		timerInfiniteFuryCD:Stop(args.destGUID)
	...
	elseif cid == 208438 then --Саботажник из рода Бесконечности
		timerTimelessCurseCD:Stop(args.destGUID)
		timerBronzeExhalationCD:Stop(args.destGUID)
	elseif cid == 208440 then --Душегуб из рода Бесконечности
...
function mod:CHAT_MSG_MONSTER_SAY(msg)
	if (msg == L.MProshlyapPrePull6 or msg:find(L.MProshlyapPrePull6)) then
		DBM:Debug("MPPR6", 2)
		self:SendSync("MPPR6") --
