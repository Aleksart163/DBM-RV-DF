...
local timerSpewCorruptionCD			= mod:NewCDNPTimer(23.2, 218755, nil, nil, nil, 3) --Выброс порчи + треш 30.3
...
local timerPropellingChargeCD		= mod:NewCDNPTimer(14, 200768, nil, nil, nil, 3) --Рывок вперед 18.2
...
local timerDreadInfernoCD			= mod:NewCDNPTimer(11, 201399, 174716, "HasInterrupt", nil, 4, nil, DBM_COMMON_L.INTERRUPT_ICON) --Жуткое пекло (Бомба) 15.8
local timerCurseofIsolationCD		= mod:NewCDNPTimer(12.4, 225568, nil, "HasInterrupt", nil, 4, nil, DBM_COMMON_L.INTERRUPT_ICON) --Проклятие уединения
...
function mod:SPELL_CAST_START(args)
	...
	elseif spellId == 200580 then --Безумный рев
		...
		if not UnitIsDeadOrGhost("player") and expireTime and self:AntiSpam(3, "MaddeningRoar") then
